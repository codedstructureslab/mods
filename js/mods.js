...
function update_module(mod){
   ...
   args.definition = def   
   args.id = idnumber
   args.top = mod.dataset.top
   args.left = mod.dataset.left
   add_module(args)
   //
   // add links
   //
...
function edit_module(evt) {
   var win = window.open('templates/edit.html')
   win.mod = evt.target.parentNode.parentNode
}
...
function receiveMessage(evt){
      ...
      mod = parser.parseFromString(evt.data, "text/html")
      update_module(mod.body.firstChild)
}
